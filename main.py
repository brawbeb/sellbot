...
# ----- Товары -----
async def add_product(seller_id, name, price, description, section, quantity=None):
    ...
    product = {
        ...
        "quantity": quantity,  # может быть строка типа "1 шт.", "3 шт.", "∞"
...
async def manage_sections_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not section:
        text = "У вас пока нет раздела. Создайте раздел (например, «Cookie», «Soft»):"
...
async def add_product_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("Введите **название** товара (например, «ROYALE HIGH 1 GP+»):", parse_mode='Markdown')
...
async def process_product_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'name':
        ...
        await update.message.reply_text("Введите **цену** товара (в RUB, только число):")
    elif state == 'price':
        try:
        ...
        await update.message.reply_text("Введите **описание** товара (краткое описание):")
    elif state == 'desc':
        ...
        if not section:
            ...
            return
        product_id = await add_product(user_id, name, price, desc, section, quantity)
...
# ----- Все товары (группировка по категориям) -----
...
async def product_back_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    product_id = int(query.data.split("_")[2])
    await product_detail_callback(update, context)
...
async def main():
    ...
    application.add_handler(CallbackQueryHandler(product_back_callback, pattern="^back_to_product_"))
