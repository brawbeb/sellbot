    ...
    if update.message:
        ...
        async def edit_field_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if field == 'name':
                ...
                keyboard = InlineKeyboardMarkup([
                    [InlineKeyboardButton("Покупатель должен предоставить данные",
                                          callback_data="edit_data_from_buyer")],
                    [InlineKeyboardButton("Продавец должен предоставить данные",
                                          callback_data="edit_data_from_seller")],
                    [InlineKeyboardButton("🔙 Отмена", callback_data="cancel_edit")]
                ])
                await query.edit_message_text("Выберите, кто должен предоставить дополнительные данные:",
                                              reply_markup=keyboard)
            else:
        ...
        async def process_custom_qty(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if not payment_card:
                await update.message.reply_text(
                    "❌ Реквизиты для оплаты ещё не заданы администратором. Попробуйте позже.")
                return
            text = (
                f"✅ Для оплаты совершите перевод по данному номеру:\n"
                f"👉 `{payment_card}` (нажмите, чтобы скопировать)\n"
        ...
        async def process_request_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            try:
                ...
                await update.message.reply_text(
                    "❌ Не удалось отправить сообщение покупателю (возможно, он заблокировал бота).")
        ...
        async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            logger.info(
                f"text_router: user={user_id}, text='{text}', state={context.user_data.get('awaiting_product')}")
        ...
        async def main():
            ...
            # Универсальный роутер для всех текстовых сообщений
